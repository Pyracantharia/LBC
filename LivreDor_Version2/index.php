...
/**
 * La pagination
 * Je prépare la requête et je compte le nombre d'element que j'ai dans la table livredor
 * Ensuite je le renvoie sous le format nbr
 */
$query = $db->getPdo()->prepare('SELECT COUNT(*) AS nbr FROM livredor');
$query->execute();
...
/**
 * Je récupère mes élèments dans ma table livredor
 ...
 * A la fin je fetchAll ça veut dire que je souhaite tout récupérer et j'appel le tout dans ma class LIVREDOR
 * le LivreDor::class c'est pour appeler la class grâce au namespace, si je ne fais pas ça ça me retourne une erreur
 */
$query = $db->getPdo()->prepare('SELECT * FROM livredor ORDER BY id DESC LIMIT :premier, :parpage');
$query->bindValue(':premier', $premier, PDO::PARAM_INT);
$query->bindValue(':parpage', $parPage, PDO::PARAM_INT);
$query->execute();
$result = $query->fetchAll(PDO::FETCH_CLASS, LivreDor::class);
...
?>
  ...
  <div class="bg-light p-5 mb-5 rounded">
    ...
    if(isset($_GET['action']) && $_GET['action'] === 'update') {
       ...
       */
      $id = (int)$_GET['id'];
      $query = $db->getPdo()->prepare('SELECT * FROM livredor WHERE id = :id');
      $query->execute([
        "id" => $id
      ]);
      $getData = $query->fetch();
